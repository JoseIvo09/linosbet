document.addEventListener('DOMContentLoaded', function() {
  ...
  function minimizeIframe() {
    ...
    myFrame.style.height = '30px'; // Ajusta a altura do iframe quando minimizado
  }

  function maximizeIframe() {
    var container = document.getElementById('iframeContainer');
    container.classList.remove('minimized');
    myFrame.style.height = '560px'; // Ajusta a altura do iframe quando maximizado
  }
  ...
  function dragElement(elmnt) {
    ...
    if (elmnt.querySelector(".header")) {
      elmnt.querySelector(".header").addEventListener('mousedown', dragMouseDown);
      elmnt.querySelector(".header").addEventListener('touchstart', dragTouchStart);
    } else {
